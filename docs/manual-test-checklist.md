...
## 7. 走失求救與即時位置（`src/pages/Lost/`、後端 `app/services/lost/`）

e2e 用假造的定位跑過流程；持續定位在 WebView 裡的實際行為只有真機看得到。
需要兩個已互加家人的帳號：**長輩**（帳號乙）與**家人**（另一支手機）。

- [ ] **A+I 開始求救**：長輩在聊天室打「我走丟了」→ 馬上收到「讓家人看到我在哪裡」卡片；
      家人收到走失通報卡，卡上有長輩的原話；長輩隨後收到「家人已經收到通知了」
- [ ] **A+I 語音求救**：長輩用語音說「我迷路了」→ 同上，不是進一般問答
- [ ] **A+I 第一次允許定位**：長輩按卡片按鈕 → 手機詢問位置權限 → 允許後畫面出現
      「已經傳給家人」；家人收到「已經收到位置」
- [ ] **A+I 持續更新**：長輩拿著手機走一段路（畫面開著）→ 家人的地圖每十幾秒跟著移動、路線變長
- [ ] **A+I 螢幕常亮**：長輩的定位頁開著不碰 → 超過手機設定的自動鎖定時間後畫面仍亮著
      （Wake Lock；不支援的 WebView 會照常鎖屏，記下是哪一台）
- [ ] **A+I 關掉畫面**：長輩鎖屏或關掉 LIFF → 約 3～4 分鐘後家人收到「位置停止更新了」，
      長輩收到請他重新打開的卡片；重新打開後家人地圖恢復更新
- [ ] **A+I 拒絕定位**：長輩在權限詢問按拒絕 → 畫面請他回聊天室；在聊天室按下方
      「傳送一次位置」→ 家人收到「已經收到位置」、地圖出現那一點
- [ ] **A+I 已找到**：家人按「已經找到了」並確認 → 長輩的定位頁變成「家人已經找到你了」；
      其他家人收到「○○已經找到○○了」
- [ ] **A 地圖圖磚**：家人頁的底圖在行動網路下載得出來（國土測繪中心 WMTS）
- [ ] **A+I 長輩看到誰在看**：家人打開地圖頁 → 15 秒內長輩的定位頁出現「○○正在看你的位置」；
      家人關掉地圖頁 → 約 1 分鐘內那一行消失
- [ ] **A+I 我要過去找**：家人按「我要過去找○○」→ 家人手機這時才詢問位置權限；長輩在 LINE
      收到「○○正在過來找你」（只收到一次，按「先不過去了」再按回來不會再收到）；長輩的
      定位頁寫出距離，地圖上出現橘色的家人標記
- [ ] **A+I 家人切去導航**：家人按「導航過去」切到 Google 地圖 → 長輩那邊仍寫「正在過來」，
      「位置更新」的時間往上跳；切回 LIFF 後恢復更新
