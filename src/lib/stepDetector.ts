 ...
 *
 * 參數來自實機校正（Android、61 Hz）。**準確度取決於怎麼帶手機**——晃動
 * 幅度差很多，同一組參數無法同時貼合兩種情境：
 *
 * | 情境 | 0.30 / 1.5 / 300ms | 0.20 / 2.0 / 400ms |
 * |---|---|---|
 * | 手持，實走 20 步 | 26（+30%） | 21（+5%） |
 * | 口袋，實走 100 步 | **84（−16%）** | 65（−35%） |
 *
 * 預設值選口袋那組：頁面必須開著才會計步，但使用者不會一路盯著螢幕走，
 * 實際情境是螢幕亮著放口袋（所以才要 Wake Lock）。手持時會多算約三成，
 * 這是已知取捨。
 *
 * 真正的解法是讓門檻隨訊號強度自適應（例如以近期峰值起伏的移動平均推導
 * `peakDelta`），這樣兩種情境都不必遷就。尚未實作——需要再一輪實機資料
 * 才能定參數，不適合在沒有量測的情況下憑空調。
 *
 * 走路波峰在手持時達 22–27（基準約 10），訊號強度從來不是問題。
 *
...
export interface DetectStepsOptions {
  ...
  /** 相鄰兩步的最短間隔（毫秒）。實機量到的步頻是每步約 845 毫秒，所以 300
   *  毫秒在口袋情境下不會誤判；手持時晃動大，同一步的次波峰有機會被多算，
   *  見檔頭的取捨說明。 */
