 ...
 *
 * 兩條規則缺一不可：
 * 1. 欄位缺席（空字串）視為「未知」而非「不是」——全庫只有 6,095/66,478
 *    筆藥證記錄顏色，把「沒記錄」當成「不符合」會讓多候選集合裡佔多數的
 *    未知候選在使用者一回答顏色就整批消失，介面卻宣稱「已收窄到這幾種」，
 *    等於用一句謊言換來一個看似可信的短清單。
...
export function appearanceValueMatches(raw: string, target: string): boolean {
  if (!raw) return true;
  return splitAppearanceValues(raw).includes(target);
}
